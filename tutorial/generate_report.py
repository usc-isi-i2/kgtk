...
def create_bar_div(df, X, Y ):
    ...
    if X=="Pagerank":
        fig = px.bar(df, x = X, y=Y, height=HEIGHT, width=WIDTH, log_x=True, labels={
                     "Pagerank": "Log(Pagerank)"
                 }).update_xaxes(categoryorder = "total descending")
    else:
        fig = px.bar(df, x = X, y=Y, height=HEIGHT, width=WIDTH).update_xaxes(categoryorder = "total descending")
...
def create_table(df):
    ...
    for row in df.itertuples():
        ...
        if(link_present):
            ...
            val = "<a href="+str(link_list[list_index])+" title= "+ str(node_name)+" target="+"_blank"+" >"+val+"</a>"
            list_index+=1

        table_html+="""<td scope="row" ><div style="height:20px;width:100%">"""+val+"</div></td>"
        for val in row[2:]:
