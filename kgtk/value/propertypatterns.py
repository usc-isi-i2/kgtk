...
class PropertyPattern:
    ...
    @classmethod
    def new(cls, node1_value: KgtkValue, label_value: KgtkValue, node2_value: KgtkValue)->'PropertyPatternRecord':
        action: Action = Action(label_value.value)

        pattern: typing.Optional[typing.Pattern] = None
        if action in (Action.NODE1_PATTERN,
                      Action.NODE2_PATTERN,
                      Action.LABEL_PATTERN,
                      Action.MATCHES):
        ...
        if action in (Action.LABEL_COLUM,
                      Action.NODE1_COLUMN,
                      Action.NODE2_COLUMN,
                      Action.REQUIRED_IN):
            if label_value.is_symbol:
        ...
        value: str = ""
        if action in (Action.NODE1_TYPE,
                      Action.NODE2_TYPE,
                      Action.ISA):
