...
def add_arguments(parser):
	...
	parser.add_argument("-s", "--subject", action="store", type=int, dest="sub", help='Column in which the subject is given, default 0', default=0)
	parser.add_argument("-o", "--object", action="store", type=int, dest="obj", help='Column in which the subject is given, default 2', default=2)
	parser.add_argument('-p', '--props', action='store', type=str, dest="props", help="Edge properties to store in their order of appearance - comma-separated string.")
	parser.add_argument('--directed', action='store_true', dest="directed", help="Is the graph directed or not?")
	parser.add_argument('-out', '--output', action='store', type=str, dest='output', help='Graph tool file to dump the graph too - if empty, it will not be saved.')
