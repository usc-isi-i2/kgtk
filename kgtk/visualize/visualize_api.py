...
def to_html(input_file: KGTKFiles,
    ...
    try:
        ...
        if edge_color_column is not None and edge_color_style == 'gradient':
            edge_color_list = []
            for edge in edges:
            ...
            for row in kr_node:
                ...
                if tooltip_column is not None:
                    if (not pd.isna(row[kr_node.column_name_map[tooltip_column]])) and str(
                        ...
                        temp['tooltip'] = str(
                            row[kr_node.column_name_map[node_file_id]])
                else:
                    ...
                    if node_color_mapping == 'fixed':
                        ...
                        if node_color_style == 'gradient':
                            if node_color_scale == 'linear':
                                ...
                                temp['color'] = row[kr_node.column_name_map[node_color_column]] if not pd.isna(
                                    row[kr_node.column_name_map[node_color_column]]) else node_color_default
                        else:
                            node_color = 2
